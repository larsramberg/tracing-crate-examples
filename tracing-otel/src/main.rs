...
fn init_tracing() {
    ...
    let subscriber = Registry::default()
        ...
        .with(metrics_layer)
        .with(get_logging_layer())
        .with(fmt_layer);

    subscriber.init();
}

fn get_logging_layer() -> OpenTelemetryTracingBridge<SdkLoggerProvider, SdkLogger> {
    let log_exporter = LogExporter::builder().with_http().build().unwrap();
